...
void MySlider::drawLinearSlider(juce::Graphics& g, int x, int	y, int width, int height, float	sliderPos, float minSliderPos,
    ...
    if (style == juce::Slider::SliderStyle::LinearHorizontal) {
        ...
        //NICO HELP ME
        juce::Path Triangle;
        Triangle.addTriangle(trackX + sliderPos, y + HorizontalOffset, 
            trackX + sliderPos + (HorizontalTriangle / 2), y + HorizontalTriangle + HorizontalOffset, 
            trackX - (HorizontalTriangle/2) - sliderPos, y + HorizontalTriangle + HorizontalOffset);

        g.fillPath(Triangle);
        // Disegna la sfera
        float sphereRadius = 15.0f;
        g.setColour(juce::Colours::lightblue);
        //.fillEllipse(sliderPos * width, sliderPos , sphereRadius, sphereRadius);
        DBG(sliderPos);
    }
